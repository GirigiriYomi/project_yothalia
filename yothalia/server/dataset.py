...
class BenchmarkDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        potential_assisstant.insert(correct_position, correct_assisstant)

        # Get prompt 
        prompt = self.get_prompt(instruction, user, potential_assisstant)

        return idx, prompt, correct_position, correct_assisstant
    
    def get_prompt(self, instruction, user, potential_assisstant):
        prompt = f"Please select the best response with the given instruction and input. Return with its No. number of response.\n" \
